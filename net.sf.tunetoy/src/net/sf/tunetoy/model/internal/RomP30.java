...
public class RomP30 extends RomOBD1 implements IRomP30 {
	class RomP30Capabilities extends RomCapabilities {
		...
		@Override
		public Boolean isEldEnabled() { return RomP30.this .readUnsignedByteAt(RomP30.RomP30Constants.sAddressEldDisable) == 0xFF; }

		@Override
		public Boolean isVtecEnabled() { return RomP30.this .readUnsignedByteAt(RomP30.RomP30Constants.sAddressVtecDisable) == 0x00; }
		...
		public RomP30NonVtecFuelMap() {
			...
			for (int map = 0; map < this.mapScalars.length; map++)
				for (int rpm = 0; rpm < this.rpmScalars.length; rpm++) {
					var addressFuelTableLocation   = Integer.valueOf(RomP30.this.addressLowCamFuelTable() + rpm * RomP30.this.getNrMapScalars() + map);
					var addressFuelMultiplierTable = Integer.valueOf(RomP30.this.addressLowCamFuelMultiplierTable() + map);
	...
	class RomP30NonVtecIgnitionMap extends RomIgnitionMap {
		public RomP30NonVtecIgnitionMap() {
			...
			for (int map = 0; map < this.mapScalars.length; map++)
				for (int rpm = 0; rpm < this.rpmScalars.length; rpm++)
					this.values[map][rpm] = (float) ((readUnsignedByteAt(RomP30.this.addressLowCamIgnitionAdvanceTable() + rpm * RomP30.this.getNrMapScalars() + map) - 24) / 4.);
		}
	}
		...
		public RomP30NonVtecLambdaMap() {
			...
			for (int map = 0; map < this.mapScalars.length; map++)
				for (int rpm = 0; rpm < this.rpmScalars.length; rpm++)
					...
					this.values[map][rpm] = (float) ((255 - readUnsignedByteAt(RomP30.this.addressLowCamLambdaTable() + rpm * RomP30.this.getNrMapScalars() + map)) * 0.123);
		}
	}
	...
	class RomP30VtecFuelMap extends RomFuelMap {
		public RomP30VtecFuelMap() {
			...
			for (int map = 0; map < this.mapScalars.length; map++)
				for (int rpm = 0; rpm < this.rpmScalars.length; rpm++) {
					var addressFuelTableLocation   = Integer.valueOf(RomP30.this.addressHighCamFuelTable() + rpm * RomP30.this.getNrMapScalars() + map);
					var addressFuelMultiplierTable = Integer.valueOf(RomP30.this.addressHighCamFuelMultiplierTable() + map);
	...
	class RomP30VtecIgnitionMap extends RomIgnitionMap {
		public RomP30VtecIgnitionMap() {
			...
			for (int map = 0; map < this.mapScalars.length; map++)
				for (int rpm = 0; rpm < this.rpmScalars.length; rpm++)
					this.values[map][rpm] = Float.valueOf((readUnsignedByteAt(Integer.valueOf(RomP30.this.addressHighCamIgnitionAdvanceTable() + rpm * RomP30.this.getNrMapScalars() + map)) - 24) / 4.f);
		}
	}
	...
	class RomP30VtecLambdaMap extends RomLambdaMap {
		public RomP30VtecLambdaMap() {
			...
			for (int map = 0; map < this.mapScalars.length; map++)
				for (int rpm = 0; rpm < this.rpmScalars.length; rpm++) {
					this.values[map][rpm] = (float) ((255 - readUnsignedByteAt(RomP30.this.addressHighCamLambdaTable() + rpm * RomP30.this.getNrMapScalars() + map)) * 0.123); // FIXME:
					// Calculations
					// are
					// off
				}
	...
	public RomP30(String filename) throws InvalidRomException, WrongRomHandlerException {
		...
		if (this.readUnsignedByteAt(0) != 226 || this.readUnsignedByteAt(1) != 33) throw new WrongRomHandlerException();
		
		this.romCaps = new RomP30Capabilities();

		if (this.getCapabilities().hasBoostTools()) this.calculateAdvancedBoostMemoryLocations();
	...
	protected void calculateAdvancedBoostMemoryLocations() {
		...
		Integer HiLmbTblCor = boostColumns * this.getVtecLambdaTableHeight()+ HiRevTblCor * 0x10;
		Integer LoLmbTblCor = boostColumns * this.getNonVtecLambdaTableHeight()+ LoRevTblCor * 0x10;
		Integer HiIgnTblCor = boostColumns * this.getVtecIgnitionTableHeight()+ HiRevTblCor * 0x10;
		Integer LoIgnTblCor = boostColumns * this.getNonVtecIgnitionTableHeight() + LoRevTblCor * 0x10;
		Integer HiFulTblCor = boostColumns * (this.getVtecFuelTableHeight() + 1) + HiRevTblCor * 0x10;
		Integer LoFulTblCor = boostColumns * (this.getNonVtecFuelTableHeight() + 1) + LoRevTblCor * 0x10;
		...
		this.addressLowCamFuelMultiplierTableBoost = this.addressLowCamFuelTableBoost+ this.getNrMapScalars() * this.getNonVtecFuelTableHeight();
		this.addressHighCamFuelMultiplierTableBoost = this.addressHighCamFuelTableBoost+ this.getNrMapScalars() * this.getVtecFuelTableHeight();
	}

	protected Integer[] getMapScalars() {
		if (this.mapScalars != null) return this.mapScalars;
		...
		if (this.getCapabilities().hasBoostTools()) {
			for (int map = 0; map < this.getNrMapScalars(); map++) {
				...
				this.mapScalars[map] = (byteval * (map_max - map_min)) / 255 + map_min;
			}
		} else {
			for (int map = 0; map < this.getNrMapScalars(); map++)
				this.mapScalars[map] = (1764 * readUnsignedByteAt(RomP30.RomP30Constants.sAddressMAPScalars + map)) / 255;
		}

		return this.getMapScalars();
	}

	protected Integer getNonVtecFuelTableHeight() { return this.readUnsignedByteAt(0x0B3C); }

	protected Integer getNonVtecIgnitionTableHeight() { return this.readUnsignedByteAt(0x0B3C); }

	protected Integer getNonVtecLambdaTableHeight() { return this.readUnsignedByteAt(0x0B3C); }
	...
	protected Integer[] getNonVtecRpmScalars() {
		...
		for (int rpm = 0; rpm < RomP30.RomP30Constants.sNrRpmScalars; rpm++) {
			...
			int value = readUnsignedByteAt(Integer.valueOf(RomP30.RomP30Constants.sAddressLowCamRPMScalars + rpm));
	...
	protected Integer getNrMapScalars() { return this.readUnsignedByteAt(0x0B3A); }

	protected Integer getNrOfExtraColumns() { return this.getNrMapScalars() - 0x10; }

	protected Integer getVtecFuelTableHeight() { return this.readUnsignedByteAt(0x0B3C); }

	protected Integer getVtecIgnitionTableHeight() { return this.readUnsignedByteAt(0x0B3C); }

	protected Integer getVtecLambdaTableHeight() { return this.readUnsignedByteAt(0x0B3C); }
	...
	protected Integer[] getVtecRpmScalars() {
		...
		Integer address = this.getCapabilities().hasBoostTools() ? 0x7102 - this.getVtecIgnitionTableHeight() + 0x14 : RomP30.RomP30Constants.sAddressHighCamRPMScalars;

		for (int rpm = 0; rpm < RomP30.RomP30Constants.sNrRpmScalars; rpm++)
			// http://www.pgmfi.org/twiki/bin/view/Library/OBD1_8bitHighCamRPM
			this.rpmVtecScalars[rpm] = (1875000 * this.readUnsignedByteAt(address + rpm)) / 53248;
