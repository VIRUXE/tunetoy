...
public class SampleView extends ViewPart {
	 ...
	 */
	@Override
	public void createPartControl(Composite composite) { this.parent = composite; }
	...
	private void initializeGUI() {
		...
		for (int i = 0; i < mapWidth + 1; i++) new TableColumn(table, SWT.CENTER);

		var tableViewer = new TableViewer(table);
		...
		tableViewer.setContentProvider(new IStructuredContentProvider() {
			...
			Object inputElement) {
				...
				for (int i = 0; i < mapHeight + 1; i++) a.add(i);
				return a.toArray();
			}
		...
		tableViewer.setLabelProvider(new ITableLabelProvider() {
			...
			int columnIndex) { return null; }

			public String getColumnText(Object element, int columnIndex) {
				Integer i = (Integer) element;
				if (i == 0) {
					if (columnIndex == 0) return new String();
					return SampleView.this.romMap.getMapScalars()[columnIndex - 1].toString();
				}

				if (columnIndex == 0) return SampleView.this.romMap.getRpmScalars()[i - 1].toString();
				
				return SampleView.this.romMap.getValue(columnIndex - 1, i - 1).toString();
			}
		...
		for (var item : table.getItems()) {
			item.setForeground(0, Display.getDefault().getSystemColor(SWT.COLOR_WHITE));
			item.setBackground(0, Display.getDefault().getSystemColor(SWT.COLOR_BLACK));
		}

		table.getItem(0).setForeground(Display.getDefault().getSystemColor(SWT.COLOR_WHITE));
		table.getItem(0).setBackground(Display.getDefault().getSystemColor(SWT.COLOR_BLACK));
	...
	public void setRom(IRom rom, String viewType) {
		if (this.currentRom != null) this.mainComposite.dispose();
		this.currentRom = rom;
